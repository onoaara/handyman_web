...
export default function AdminSupervisors() {
  ...
  const users = filterUsersByRole(data ?? [], "supervisor");

  const errorMessage =
    error && typeof error === "object" && "error" in error
      ? String(
          (error as { error?: unknown }).error ?? "Failed to load supervisors"
        )
      : error
  ...
  return (
    <div>
      <h2 className="mb-4 text-2xl font-semibold">Supervisors</h2>
        ...
        <button
          ...
          onClick={async () => {
            try {
              ...
              toast.success("Supervisors refreshed");
            } catch (e) {
              const message =
                e && typeof e === "object" && "error" in e
                  ? String(
                      (e as { error?: unknown }).error ??
                        "Failed to refresh supervisors"
                    )
                  : "Failed to refresh supervisors";
              toast.error(message);
            }
